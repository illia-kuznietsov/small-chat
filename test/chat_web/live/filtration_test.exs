defmodule ChatWeb.FiltrationTest do
  ...
  test "test filters" do
    ...
    ChatWeb.Message.create_message("testuser_v2", "testmessage2") |> ChatWeb.Storage.post_message()
    ChatWeb.Message.create_message("testuser_v3", "testmessage3") |> ChatWeb.Storage.post_message()
    ...
    assert length(ChatWeb.Filtration.filter_messages("", %{"20-percent-minority-most-liked" => "on"})) == 3
    ChatWeb.Storage.update_message_likes(second.id, "testuser_v1")
    ChatWeb.Storage.update_message_likes(third.id, "testuser_v1")
    assert length(ChatWeb.Filtration.filter_messages("", %{"20-percent-minority-most-liked" => "on"})) == 2
  end
end
